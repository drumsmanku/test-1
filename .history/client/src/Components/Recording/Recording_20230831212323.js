...
function Recording() {
  ...
  const [secondsScreenRecorded, setSecondsScreenRecorded] = useState(0);

  let intervalId = useRef(null);
  let screenIntervalId = useRef(null);
...
const startScreenRecording = () => {
  if (screenMediaRecorder) {
    ...
    screenIntervalId.current = setInterval(() => {
      setSecondsScreenRecorded((prevSeconds) => prevSeconds + 1);
...
const stopScreenRecording = async () => {
  ...
  return (
    <div className={styles.container}>
      ...
      <div>
        Time recorded for Screen Recording
        <p style={{backgroundColor:'beige', borderRadius:'0.3rem', border:'2px solid black', padding:'1rem'}}>{secondsScreenRecorded}s</p>
      </div>
