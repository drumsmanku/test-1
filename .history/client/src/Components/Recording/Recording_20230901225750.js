...
import { uploadToCloud } from '../uploadUtil';
...
function Recording() {
  ...
  //     .then((response)=> {setWebcamVideoUrl(response.data.webcamVideoUrl); console.log(response.data);});
  
  //     await axios.get('http://localhost:4000/recording/screendata', {recordingId: screenRecordingId})
  //     .then((response)=>{setScreenVideoUrl(response.data.screenVideoUrl); console.log(response.data);});
  //   }
  //   getData();
  ...
  const stopRecordingWebcam = async () => {
    ...
    if (mediaRecorderWebcam) {
        ...
        await uploadToCloud(blobWebcam, webcamRecordingId, sampleVar).then(axios.get('http://localhost:4000/recording/webcamdata', {recordingId:webcamRecordingId})
        .then((response)=> {setWebcamVideoUrl(response.data.webcamVideoUrl); console.log(webcamVideoUrl)})); 
...
const stopRecordingScreen = async () => {
  ...
  if (mediaRecorderScreen) {
    ...
    await uploadToCloud(blobScreen, screenRecordingId,sampleVar).then(axios.get('http://localhost:4000/recording/screendata', {recordingId: screenRecordingId})
    .then((response)=>{setScreenVideoUrl(response.data.screenVideoUrl); console.log(screenVideoUrl);}));
