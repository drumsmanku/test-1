...
function Recording() {
  ...
  useEffect(() => {
    async function getData() {
      try {
        const webcamResponse = await axios.get('http://localhost:4000/recording/webcamdata', {recordingId: webcamRecordingId});
        setWebcamVideoUrl(webcamResponse.data.webcamVideoUrl);
        
        const screenResponse = await axios.get('http://localhost:4000/recording/screendata', {recordingId: screenRecordingId});
        setScreenVideoUrl(screenResponse.data.screenVideoUrl);
